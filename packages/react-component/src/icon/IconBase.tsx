...
const IconBase = React.forwardRef<SVGSVGElement, IconBaseProps>((props, ref) => {
  const { name = '', className, style, spin, rotate, tabIndex, onClick, children, ...restProps } = props
  ...
  const attrs = {
    ...
    onClick,
    'className': ['rc-icon', kebabCaseName ? `rc-icon-${kebabCaseName}` : '', spin ? 'rc-icon--spin' : '', className]
      .join(' ')
      .trim(),
